# Final report — run `1446/12_paper`
...
29 event(s) at normal priority over 88.4 s analysed; 0 retained at low priority and 0 excluded from automatic ordering. Priority is a reading order, not a probability that anything occurred.
...
| 04_calibration  | complete            | campaper/e1/v1 (draft)                                                 |
...
- **233 of 5881 crops** (4.0%) fall below the native-resolution floor. No detector can answer for these.
- Median crop magnification **7.34x**, maximum **21.46x**. Magnification is interpolation, not detail.
- Crop policy mix: {'native_source_context_fallback': 3499, 'pose_hand_context': 2382}.

- Crops fell back from `pose_hand_context` for: {'no_pose_observation': 1637, 'wrist_confidence_below_floor': 1588, 'wrist_not_resolvable_at_source_scale': 274}.

## Evidence packets

29 of 29 packets meet PRD 11's composition requirements. an invalid packet is never sent to a model. PRD 11 prohibits whole-frame-only packets because a frame showing thirty candidates and marking none invites a confident description of the wrong person.
